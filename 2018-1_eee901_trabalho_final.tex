...
\usepackage{amssymb} % Símbolos de conjunto reais R, naturais N, inteiros Z, etc
\usepackage{enumerate} % Alterar o índice do enumerate
\usepackage{cite}
...
\title{Utiliza\c{c}\~ao de t\'ecnicas de intelig\^encia computacional para estimar a press\~ao arterial a partir da fotopletismografia.}


% author names and affiliations
% use a multiple column layout for up to three different
% affiliations
\author{\IEEEauthorblockN{Alan Souza, Alex Assis, 	Douglas Marques, Roger Ferreira}
\IEEEauthorblockA{Departamento de P\'{o}s Gradua\c{c}\~{a}o em Engenharia El\'{e}trica\\
Universidade Federal de Minas Gerais\\
Email: alancsouza04@gmail.com, assis.alex.d@gmail.com, tatudoug@gmail.com, rofes31@gmail.com }}
...
%O PPG tem sido usado para medi\c{c}\~ao de batimentos card\'iacos, propriedades vasculares perif\'ericas, press\~ao arterial e atividades nervosas aut\^onomas. \cite{GEUN:2008}
%Uma grande limita\c{c}\~ao pr\'atica \'e o registro de baixo n\'ivel de PPG registrados em \'areas vasculares de baixa densidade da pele.\\
...
%Entretanto, por questões de implementação computacional, considera-se a Transformada Discreta de Fourier. 
%
%Dado um elemento $n$-dimensional $G_{j_1,j_2\cdots j_n}$ escrito $G_\mathbf{j}$, onde $\mathbf j$ é um vetor $n$ dimensional de valores inteiros, com $0 \le j_i < N_i$.  Podemos definir então a soma:
%
%\begin{equation}
%  \sum_{\mathbf{j}} \equiv
%  \sum_{j_1=0}^{N_1-1}\sum_{j_2=0}^{N_2-1}\cdots\sum_{j_n=0}^{N_n-1}
%\end{equation}
%Através dessa notação, podemos escrever a Transformada discreta $n$-dimensional de Fourier como segue:
%
%\begin{equation}
%  \label{ft_discrete}
%  \hat{G}_{\mathbf{k}} 
...
%\end{equation}
%
%E sua inversa correspondente, dada por:
%
%\begin{equation}
%  \label{ift_discrete}
%  G_{\mathbf{j}}
...
%\end{equation}
%
%As expressões nas equações \ref{ft_discrete} e \ref{ift_discrete} podem ser implementadas de forma computacionalmente eficiente através da utilização da Transformada Rápida de Fourier (FTT).
%
%\subsection{Transformada Wavelet}
%
%A Transformada \textit{Wavelet} têm sido utilizada em diversas aplicações de processamento de sinais, dada sua habilidade de decompor o sinal em componentes de frequência espacialmente distribuídos os quais podem ser filtrados e selecionados de acordo com a necessidade. Dentre as aplicações destacam-se a redução de ruídos, compactação de dados e extração de características \cite{leavey2003introduction}. \textit{Wavelets} são funções matemáticas que apresentam comportamento oscilatório. Diferentemente das funções senoidais, cuja oscilação está presente em todo sinal, \textit{wavelets} apresentam oscilações localizadas e a análise de sinais através dessa transformada se dá pela da quebra do mesmo em diferentes componentes de frequência através da comparação do sinal com funções \textit{wavelet} de diversos tamanhos.
...
%
%%\bibitem{livro:HAYKIN}
%%HAYKIN, S. \textbf{Neural Networking and Learning Machines}. 3rd ed. Pearson Prentice Hall, 2008.\\
%
%
%
%
